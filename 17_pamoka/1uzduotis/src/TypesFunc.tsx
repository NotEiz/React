...
const addNumbers = (num1: number, num2: number): number => {
   ...
   */
   ...
   {
     type oneNumberType = (number:number) => number;
     type multiplyTypes = (a:number, b:number) => number ;
     type singleStringType = (text:string) => void; 
     const multiply:multiplyTypes = (a, b) => {
       return a * b;
     };
   
     const power:multiplyTypes = function (base, power) {
       return base ** power;
     };
   
     const squareRoot:oneNumberType = (number) => number ** (1 / 2);
   
     const root:multiplyTypes = function (base, nthRoot) {
       return base ** (1 / nthRoot);
     }
   
     const printBlueText:singleStringType = (text) => console.log(`%c${text}`, 'color: #0000ee');
